...
- **Coverage**: To see the test coverage of the test-files with their corresponding programs that were tested, execute `pytest --cov=vm --cov=assembler --cov=test_assembler --cov=test_vm --cov=test_errors` in the terminal. Since the generated files and output were compared in the tests, the test-coverage is not really meaningful. An informative and meaningful test-coverage will be seen in task 2.
